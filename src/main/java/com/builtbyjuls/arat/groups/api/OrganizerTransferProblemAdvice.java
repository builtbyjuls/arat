...
class OrganizerTransferProblemAdvice {

    private final ApiProblemFactory problemFactory;
    private final ProblemResponseWriter responseWriter;

    OrganizerTransferProblemAdvice(ApiProblemFactory problemFactory, ProblemResponseWriter responseWriter) {
        this.problemFactory = problemFactory;
        this.responseWriter = responseWriter;
    }

    @ExceptionHandler(GroupVersionPreconditionException.class)
    void preconditionFailure(
            GroupVersionPreconditionException exception,
    ...
    @ExceptionHandler(OrganizerTransferException.class)
    void transferFailure(
            OrganizerTransferException exception,
            HttpServletRequest request,
            HttpServletResponse response) throws IOException {
        switch (exception.reason()) {
            ...
            case ALREADY_ORGANIZER -> write(request, response, HttpStatus.CONFLICT,
                    "ALREADY_ORGANIZER", "Already organizer", "The target is already an organizer.");
            case PRECONDITION_FAILED -> write(request, response, HttpStatus.PRECONDITION_FAILED,
                    "PRECONDITION_FAILED", "Precondition failed", "The group version no longer matches If-Match.");
        }
    }
