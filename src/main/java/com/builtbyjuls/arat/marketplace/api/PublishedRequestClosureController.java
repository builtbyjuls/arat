...
import com.builtbyjuls.arat.marketplace.application.RequestClosureService;
...
@RestController
@RequestMapping("/api/v1/published-requests")
@Validated
@Tag(name = "Published requests")
public class PublishedRequestClosureController {

    private final CurrentActor currentActor;
    private final RequestClosureService closureService;

    public PublishedRequestClosureController(CurrentActor currentActor, RequestClosureService closureService) {
        this.currentActor = currentActor;
        this.closureService = closureService;
    }

    @PostMapping("/{requestId}/closure")
    @Operation(operationId = "closeProviderRequest", summary = "Close a provider request")
    @SecurityRequirement(name = "bearerAuth")
    @ApiResponses({
        @ApiResponse(
                responseCode = "200",
                description = "Provider request closed",
                headers = @Header(name = "ETag", description = "New plan version"),
        ...
        @ApiResponse(responseCode = "404", description = "Private resource not found",
                content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE,
                        schema = @Schema(implementation = ApiProblemResponse.class))),
        @ApiResponse(responseCode = "409", description = "Request is not open or idempotency key was reused",
                content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE,
                        schema = @Schema(implementation = ApiProblemResponse.class))),
    ...
    public ResponseEntity<PublishedRequestRepresentation> close(
        ...
        var response = closureService.close(new ClosePublishedRequestCommand(
                actor.accountId(),
                requestId,
