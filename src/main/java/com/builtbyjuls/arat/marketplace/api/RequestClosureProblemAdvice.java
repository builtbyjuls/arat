...
@RestControllerAdvice(assignableTypes = PublishedRequestClosureController.class)
class RequestClosureProblemAdvice {

    private final ApiProblemFactory problemFactory;
    private final ProblemResponseWriter responseWriter;

    RequestClosureProblemAdvice(ApiProblemFactory problemFactory, ProblemResponseWriter responseWriter) {
    ...
    @ExceptionHandler(RequestClosureException.class)
    void closureFailure(
            RequestClosureException exception,
            HttpServletRequest request,
            HttpServletResponse response) throws IOException {
        switch (exception.reason()) {
            ...
            case INVALID_REQUEST_STATE -> write(request, response, HttpStatus.CONFLICT,
                    "INVALID_REQUEST_STATE", "Invalid request state",
                    "The provider request cannot be closed in its current state.");
        }
    }
