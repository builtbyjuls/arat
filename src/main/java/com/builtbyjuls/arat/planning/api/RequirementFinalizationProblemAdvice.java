...
@RestControllerAdvice(assignableTypes = RequirementFinalizationController.class)
class RequirementFinalizationProblemAdvice {
    private final ApiProblemFactory problemFactory;
    private final ProblemResponseWriter responseWriter;
    RequirementFinalizationProblemAdvice(ApiProblemFactory problemFactory, ProblemResponseWriter responseWriter) {
        this.problemFactory = problemFactory;
        this.responseWriter = responseWriter;
    }
    @ExceptionHandler(PlanVersionPreconditionException.class)
    void preconditionFailure(PlanVersionPreconditionException exception, HttpServletRequest request, HttpServletResponse response) throws IOException {
        switch (exception.reason()) {
            case PRECONDITION_REQUIRED -> write(request, response, HttpStatus.PRECONDITION_REQUIRED, "PRECONDITION_REQUIRED", "Precondition required", "The If-Match header is required.");
            case INVALID_PRECONDITION -> write(request, response, HttpStatus.BAD_REQUEST, "INVALID_PRECONDITION", "Invalid precondition", "The If-Match header must be a quoted positive integer.");
        }
    }
    @ExceptionHandler(RequirementFinalizationException.class)
    void finalizationFailure(RequirementFinalizationException exception, HttpServletRequest request, HttpServletResponse response) throws IOException {
        switch (exception.reason()) {
